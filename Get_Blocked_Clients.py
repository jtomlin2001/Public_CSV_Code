...
subject = 'Clients Currently being blocked'
signature = """
    <html>
    <head>
    <style>
...
for network in networks:
    ...
    response = requests.get(f"https://api.meraki.com/api/v1/networks/{network_id}/policies/byClient", headers=headers)
        ...
        if clients:
            for client in clients:
                if client['assigned']:
                  ...
                  device_info.append(f"Site: {network['name']} | Device Name:{client['name']}")
        else:
            pass
header = (f'<span style="font-size: 27px;">Searching active Clients</span><br><br>')
if staff_client_count:
    clients = (f'<span style="font-size: 24px;">{staff_client_count} Blocked Clients</span><br><br>')
    for device in device_info:
        clients += (f'<li style="font-size: 17px"> {device}</li>')
else:
    clients = (f'<span style="font-size: 24px;">No Blocked clients found</span><br><br>')
clients += '<br><br>'
msg = EmailMessage()
