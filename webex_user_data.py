...
import time
from tinydb import TinyDB, Query
db = TinyDB('C:/Users/so_tomlinj/Documents/Coding/db_teams_webex_data.json')
starter = time.time()
## Gathers users list from teams  using the kern all staff and fresno all staff
def get_user_list():
    ...
    # Loop for kern "while url" loops until list is done and adds to data base 
    while url:
        info = requests.get(url, headers=headers)
        info.raise_for_status()
        user_data = info.json()
        for user in user_data['value']:
            db.insert({'displayName': user['displayName'],  'title': user['jobTitle'], 'location': user['officeLocation']})
        url = user_data.get('@odata.nextLink')
    # Loop for fresno "while url" loops until list is done and adds to data base
    while url1:
        info = requests.get(url1, headers=headers)
        info.raise_for_status()
        user_data = info.json()
        for user in user_data['value']:
            db.insert({'displayName': user['displayName'], 'title': user['jobTitle']})
        url1 = user_data.get('@odata.nextLink')
    end_time = time.time()
    print(f"Gathered Users List from Teams, Elapsed Time: {end_time - start_time} Seconds")


## residual from other code using as template
...
## Grabs display name and grabs the display name entry from the data base and add an extension to the entry
def update_webex():
    start_time = time.time()
    ...
    for person in people['items']:
        try:
            numbers = person['phoneNumbers']
            for number in numbers:
                if number['type'] == 'work_extension':
                    db.update({'extension': number['value']}, Query().displayName == person['displayName'])
        except:
            pass
    if 'Link' in people_request.headers and 'rel="next"' in people_request.headers['Link']:
        ...
        for person in people['items']:
            try:
                ...
                pass
    end_time = time.time()
    print(f"Gathered Extensions from webex, Elapsed Time: {end_time - start_time} Seconds")
